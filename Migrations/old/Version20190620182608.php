 ...
 */
final class Version20190620182608 extends AbstractMigration {
    ...
    public function down(Schema $schema): void {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__lease_request AS SELECT id, author_id, title, slug, summary, published_at, start_date, end_date, association_type, price, status, num_attendants, association, contract, contract_signed, read, paid, key_deliver, key_return FROM lease_request');
        $this->addSql('DROP TABLE lease_request');
        $this->addSql('CREATE TABLE lease_request (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, author_id INTEGER DEFAULT NULL, title VARCHAR(255) NOT NULL, slug VARCHAR(255) NOT NULL, summary VARCHAR(255) NOT NULL, published_at DATETIME NOT NULL, start_date DATETIME DEFAULT NULL, end_date DATETIME DEFAULT NULL, association_type VARCHAR(255) NOT NULL, price DOUBLE PRECISION DEFAULT NULL, status VARCHAR(255) DEFAULT NULL, num_attendants INTEGER NOT NULL, association VARCHAR(255) NOT NULL, contract VARCHAR(255) DEFAULT NULL, contract_signed VARCHAR(255) DEFAULT NULL, read BOOLEAN DEFAULT NULL, paid DOUBLE PRECISION DEFAULT NULL, key_deliver DATETIME DEFAULT NULL, key_return DATETIME DEFAULT NULL)');
        $this->addSql('INSERT INTO lease_request (id, author_id, title, slug, summary, published_at, start_date, end_date, association_type, price, status, num_attendants, association, contract, contract_signed, read, paid, key_deliver, key_return) SELECT id, author_id, title, slug, summary, published_at, start_date, end_date, association_type, price, status, num_attendants, association, contract, contract_signed, read, paid, key_deliver, key_return FROM __temp__lease_request');
        $this->addSql('DROP TABLE __temp__lease_request');
        $this->addSql('CREATE INDEX IDX_9BE13C3CF675F31B ON lease_request (author_id)');
        $this->addSql('DROP INDEX IDX_5ACE3AF08D93D649');
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, full_name, username, email, password, roles, address, phone, password_reset FROM user');
        ...
        , address CLOB NOT NULL, phone CLOB NOT NULL, password_reset CLOB DEFAULT NULL)');
        $this->addSql('INSERT INTO user (id, full_name, username, email, password, roles, address, phone, password_reset) SELECT id, full_name, username, email, password, roles, address, phone, password_reset FROM __temp__user');
