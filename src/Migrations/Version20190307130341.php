 ...
 */
final class Version20190307130341 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, full_name, username, email, password, roles, address, phone FROM user');
        ...
        , address CLOB NOT NULL, phone CLOB NOT NULL)');
        $this->addSql('INSERT INTO user (id, full_name, username, email, password, roles, address, phone) SELECT id, full_name, username, email, password, roles, address, phone FROM __temp__user');
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__lease_request AS SELECT id, author_id, title, slug, summary, published_at, start_date, end_date, association_type, price, status, num_attendants, association FROM lease_request');
        $this->addSql('DROP TABLE lease_request');
        $this->addSql('CREATE TABLE lease_request (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, author_id INTEGER DEFAULT NULL, title VARCHAR(255) NOT NULL COLLATE BINARY, slug VARCHAR(255) NOT NULL COLLATE BINARY, summary VARCHAR(255) NOT NULL COLLATE BINARY, published_at DATETIME NOT NULL, start_date DATETIME DEFAULT NULL, end_date DATETIME DEFAULT NULL, association_type VARCHAR(255) NOT NULL COLLATE BINARY, price DOUBLE PRECISION DEFAULT NULL, status VARCHAR(255) DEFAULT NULL COLLATE BINARY, num_attendants INTEGER NOT NULL, association VARCHAR(255) NOT NULL COLLATE BINARY, contract VARCHAR(255) DEFAULT NULL, CONSTRAINT FK_9BE13C3CF675F31B FOREIGN KEY (author_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO lease_request (id, author_id, title, slug, summary, published_at, start_date, end_date, association_type, price, status, num_attendants, association) SELECT id, author_id, title, slug, summary, published_at, start_date, end_date, association_type, price, status, num_attendants, association FROM __temp__lease_request');
    ...
    public function down(Schema $schema): void
    {
        ...
        , address CLOB DEFAULT \'""\' NOT NULL COLLATE BINARY, phone CLOB DEFAULT \'""\' NOT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO user (id, full_name, username, email, password, roles, address, phone) SELECT id, full_name, username, email, password, roles, address, phone FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_8FB094A1E7927C74 ON user (email)');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_8FB094A1F85E0677 ON user (username)');
